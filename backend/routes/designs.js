...
router.post('/', protect, async (req, res) => {
  console.log('[Save Design] body keys:', Object.keys(req.body));
  const {
    prompt,
    negativePrompt,
    imageDataUrl,
...
router.get('/', protect, async (req, res) => {
  try {
    ...
    const q = Design.find({ user: req.user.id })
      ...
      .skip(skip)
      .limit(limit)
      .select('_id prompt negativePrompt publicUrl thumbUrl imageDataUrl settings isSubmittedForContest contestSubmissionMonth votes createdAt')
      .lean();
