...
const CORE_COLORS = [ /* ... Full list of colors ... */ ];
const initialColorVariantState = { colorName: '', colorHex: '', podProductId: '', isDefaultDisplay: false, imageSet: [{ url: '', isPrimary: true }], sizes: [] };
...
const ProductManager = () => {
  ...
  const fetchProductTypes = useCallback(async () => { if (!token) return []; try { const { data } = await client.get('/admin/product-types', { headers: { Authorization: `Bearer ${token}` } }); setProductTypes(data); return data; } catch (e) { toast({ title: "Error", description: "Could not load product types.", status: "error" }); return []; } }, [token, toast]);
  ...
  const handleSubmit = async () => {
    if (!formData.name.trim() || !formData.productType) { toast({ title: "Validation Error", status: "error" }); return; }
    for (const variant of formData.variants) { for (const image of variant.imageSet) { if (!image.url || image.url.trim() === '') { toast({ title: "Image URL Missing", description: `Please provide a URL for all images in the "${variant.colorName}" gallery.`, status: "error" }); return; } } }
  ...
  return (
    <Box p={{ base: 2, md: 4 }}>
      ...
      <TableContainer>
        <Table variant="simple" size="sm"><Thead><Tr><Th>Name</Th><Th>Type</Th><Th>Base Price</Th><Th>Variants</Th><Th>Status</Th><Th>Actions</Th></Tr></Thead>
      ...
      <Modal isOpen={isOpen} onClose={onClose} size="6xl" scrollBehavior="inside">
        <ModalOverlay />
        <ModalContent><ModalHeader>{isEditing ? 'Edit' : 'Add New'} Product</ModalHeader><ModalCloseButton />
          <ModalBody pb={6}>
                            ...
                            <AccordionItem border="none">
                              ...
                              <AccordionPanel bg="white" pb={4}>
                                ...
                                <RadioGroup onChange={(idx) => setPrimaryImage(colorIndex, parseInt(idx))} value={variant.imageSet?.findIndex(img => img.isPrimary)?.toString() ?? "-1"}>
                                    <VStack align="stretch" spacing={2}>{variant.imageSet?.map((img, imgIndex) => (
                                        <HStack key={imgIndex}>
                                          <Radio value={imgIndex.toString()} colorScheme="green"/>
                                          <Input flex="1" size="sm" placeholder="https://image.url/shirt.png" value={img.url} onChange={(e) => handleImageSetUrlChange(colorIndex, imgIndex, e.target.value)} />
                                          <Image src={img.url} alt="Preview" boxSize="32px" objectFit="cover" borderRadius="sm" bg="gray.100" fallback={<Icon as={FaImage} color="gray.300" boxSize="32px" p={2}/>}/>
                                          <ChakraIconButton size="sm" icon={<Icon as={FaTrashAlt}/>} onClick={() => removeImageFromSet(colorIndex, imgIndex)} isDisabled={variant.imageSet.length <= 1} />
                                        </HStack>
                                    ))}</VStack>
                                </RadioGroup>
