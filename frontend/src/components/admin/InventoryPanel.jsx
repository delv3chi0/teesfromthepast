...
const initialColorVariantState = {
    ...
    imageSet: [{ url: '', isPrimary: true, frontMockupUrl: '', backMockupUrl: '', sleeveMockupUrl: '' }], // UPDATED
    printAreas: [],
};
...
const InventoryPanel = () => {
            ...
            if (product) {
                ...
                setProductFormData({
                    ...
                    variants: (fullProductData.variants || []).map(v => ({
                        ...v,
                        imageSet: v.imageSet && v.imageSet.length > 0 ? v.imageSet : [{ url: '', isPrimary: true, frontMockupUrl: '', backMockupUrl: '', sleeveMockupUrl: '' }], // UPDATED
                        sizes: v.sizes || [],
    ...
    // UPDATED: imageSet URL handlers
    const handleImageSetUrlChange = (colorIndex, imageIndex, field, url) => {
        const newVariants = [...productFormData.variants];
        newVariants[colorIndex].imageSet[imageIndex][field] = url;
        setProductFormData(prev => ({ ...prev, variants: newVariants }));
    };
    
    const addImageToSet = (colorIndex) => { const newVariants = [...productFormData.variants]; newVariants[colorIndex].imageSet.push({ url: '', isPrimary: false, frontMockupUrl: '', backMockupUrl: '', sleeveMockupUrl: '' }); setProductFormData(prev => ({ ...prev, variants: newVariants })); };
    ...
    const handleAddAllPrintAreas = (colorIndex) => { // NEW FUNCTION
      const newVariants = [...productFormData.variants];
      const existingPlacements = new Set(newVariants[colorIndex].printAreas.map(pa => pa.placement));
      const newAreasToAdd = PREDEFINED_PRINT_AREAS.filter(pa => !existingPlacements.has(pa.placement));
      newVariants[colorIndex].printAreas.push(...newAreasToAdd);
      setProductFormData(prev => ({ ...prev, variants: newVariants }));
      toast({ title: "Predefined Print Areas Added", description: "All predefined print areas have been added to this variant.", status: "success" });
    }
    ...
    const handleProductSubmit = async () => {
        ...
        // UPDATED: Check for specific mockup URLs
        for (const variant of productFormData.variants) {
          for (const image of variant.imageSet) {
            if (!image.frontMockupUrl && !image.backMockupUrl && !image.sleeveMockupUrl) {
                toast({ title: "Image URL Missing", description: `Please provide at least one mockup URL (Front, Back, or Sleeve) for each image in the "${variant.colorName}" variant gallery.`, status: "error" });
                return;
            }
          }
        }
                                                    ...
                                                    <AccordionPanel bg="brand.secondary" pb={4}>
                                                        ...
                                                        <RadioGroup onChange={(idx) => setPrimaryImage(colorIndex, parseInt(idx))} value={variant.imageSet?.findIndex(img => img.isPrimary)?.toString() ?? "-1"}>
                                                            <VStack align="stretch" spacing={2}>
                                                                ...
                                                                <Box key={imgIndex} p={2} borderWidth="1px" borderRadius="md" borderColor="gray.600">
                                                                    <HStack mb={2} alignItems="center">
                                                                        <Radio value={imgIndex.toString()} colorScheme="green"/>
                                                                        <Text fontWeight="bold">Image #{imgIndex + 1}</Text>
                                                                        <Spacer/>
                                                                        <ChakraIconButton size="xs" icon={<Icon as={FaTrashAlt}/>} onClick={() => removeImageFromSet(colorIndex, imgIndex)} isDisabled={variant.imageSet.length <= 1} />
                                                                    </HStack>
                                                                    {/* NEW MOCKUP URL INPUTS */}
                                                                    <SimpleGrid columns={{ base: 1, md: 3 }} spacing={2} mt={2}>
                                                                        <FormControl>
                                                                            <FormLabel fontSize="sm">Front Mockup URL</FormLabel>
                                                                            <Input flex="1" size="sm" placeholder="https://image.url/front.png" value={img.frontMockupUrl} onChange={(e) => handleImageSetUrlChange(colorIndex, imgIndex, 'frontMockupUrl', e.target.value)} />
                                                                        </FormControl>
                                                                        <FormControl>
                                                                            <FormLabel fontSize="sm">Back Mockup URL</FormLabel>
                                                                            <Input flex="1" size="sm" placeholder="https://image.url/back.png" value={img.backMockupUrl} onChange={(e) => handleImageSetUrlChange(colorIndex, imgIndex, 'backMockupUrl', e.target.value)} />
                                                                        </FormControl>
                                                                        <FormControl>
                                                                            <FormLabel fontSize="sm">Sleeve Mockup URL</FormLabel>
                                                                            <Input flex="1" size="sm" placeholder="https://image.url/sleeve.png" value={img.sleeveMockupUrl} onChange={(e) => handleImageSetUrlChange(colorIndex, imgIndex, 'sleeveMockupUrl', e.target.value)} />
                                                                        </FormControl>
                                                                    </SimpleGrid>
                                                                </Box>
                                                                ))}
                                                        ...
                                                        <Button size="sm" mt={3} onClick={() => addImageToSet(colorIndex)} leftIcon={<FaPlus/>}>Add Image Set</Button>
