...
import { Link as RouterLink, useLocation } from 'react-router-dom'; // Removed useNavigate as it's not directly used in this file
      ...
      <VStack spacing={3} align="stretch" px="4" mt={8}>
        {navItems.map((item) => (
          <ChakraLink
            ...
            onClick={onClick} // For mobile drawer: close on click
          >
            {item.label}
          </ChakraLink>
        ))}
      ...
      <Box as="section" display="flex" flexGrow={1}> 
        ...
        <Drawer isOpen={isOpen} placement="left" onClose={onClose} returnFocusOnClose={false}>
          ...
          <DrawerContent bg="brand.primary" color="brand.textLight"> 
            ...
            <DrawerBody p={0}> {/* Remove padding if SidebarContent has its own */}
              <SidebarContent onClick={onClose}/> {/* Pass onClose to close drawer on nav item click */}
            </DrawerBody>
          </DrawerContent>
        </Drawer>
            ...
            <Flex align="center">
              ...
              <ChakraLink as={RouterLink} to="/dashboard" display="flex" alignItems="center" _hover={{textDecoration: "none"}}>
                <Image 
                  ...
                  objectFit="contain"
                />
                <Text fontSize="xl" fontWeight="bold" color="brand.primaryDark" display={{ base: 'none', lg: 'block' }} ml={3}>
                  Tees From The Past
                </Text>
              </ChakraLink>
            </Flex>
