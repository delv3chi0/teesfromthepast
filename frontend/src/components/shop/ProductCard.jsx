...
import {
  Box, VStack, HStack, Heading, Text, Button, AspectRatio, Image, Tooltip, Wrap, WrapItem,
...
const toKey = (c) => String(c||"").trim().toLowerCase().replace(/\s+/g," ");
    ...
    <Wrap spacing="6px" mb={2}>
      {list.map((c) => {
        ...
        return (
          <WrapItem key={c}>
            <Tooltip label={c}>
              <Box borderRadius="full" boxSize="12px" borderWidth="1px" borderColor="blackAlpha.600" background={hex} />
            </Tooltip>
          </WrapItem>
        );
      })}
...
export default function ProductCard({ product }) {
  ...
  return (
    <VStack layerStyle="cardBlue" p={4} spacing={3} align="stretch" borderRadius="xl" borderWidth="1px"
      borderColor="transparent" _hover={{ borderColor: "brand.accentYellow" }} transition="all .2s">
