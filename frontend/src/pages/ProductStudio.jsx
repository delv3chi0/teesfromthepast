// frontend/src/pages/ProductStudio.jsx (Baby Step 1: Mockup Toggle)
...
import {
    ...
    Flex, Tooltip, AspectRatio, RadioGroup, Stack // Added RadioGroup and Stack for mockup toggle
} from '@chakra-ui/react';
import { useNavigate, useLocation, Link as RouterLink } from 'react-router-dom';
import { client } from '../api/client';
import { useAuth } from '../context/AuthProvider';
import { FaShoppingCart, FaTshirt, FaPalette, FaPaintBrush } from 'react-icons/fa';
...
const ThemedSelect = (props) => (
    <Select
        ...
        // Text color should be handled by index.css now
        {...props}
    />
);
...
export default function ProductStudio() {
    ...
    // REINTRODUCED: currentMockupType state for the toggle
    const [currentMockupType, setCurrentMockupType] = useState('tee'); // Default to 'tee'
    ...
    // activeObjectRef is not needed in this simplified version
    ...
    useEffect(() => {
        const initializeAndRenderCanvas = () => {
            ...
            if (!FCanvas || !window.fabric) {
                setTimeout(initializeAndRenderCanvas, 100);
                return;
            }

            FCanvas.clear();

            // Determine mockup source based on currentMockupType (reintroduced logic)
            let mockupSrc = '';
            const imageSet = finalVariant?.imageSet; // Access imageSet with optional chaining
            const teeMockupImage = imageSet ? imageSet.find(img => img.url.includes('tee_')) : null;
            const manMockupImage = imageSet ? imageSet.find(img => img.url.includes('man_')) : null; // Kept for toggle logic
            ...
            if (currentMockupType === 'tee' && teeMockupImage) { // Prioritize tee_ if selected
                mockupSrc = teeMockupImage.url;
            } else if (currentMockupType === 'man' && manMockupImage) { // Use man_ if selected and available
                mockupSrc = manMockupImage.url;
            } else if (primaryImageFound) { // Fallback to primary if currentType not found
                mockupSrc = primaryImageFound.url;
            } else if (teeMockupImage) { // Fallback to tee_ if primary not found
                mockupSrc = teeMockupImage.url;
            } else if (manMockupImage) { // Fallback to man_ if tee_ and primary not found
                mockupSrc = manMockupImage.url;
            ...
            if (mockupSrc) {
                window.fabric.Image.fromURL(mockupSrc, (img) => {
                    FCanvas.setBackgroundImage(img, FCanvas.renderAll.bind(FCanvas), {
                        ...
                        alignX: 'center',
                        alignY: 'center',
                        meetOrSlice: 'meet' // Ensure aspect ratio is maintained (Fix for stretched man_ image)
                    });
                }, { crossOrigin: 'anonymous' });
            } else {
    ...
    }, [selectedDesign, finalVariant, currentMockupType]); // currentMockupType is now a dependency
    ...
    // Removed all custom tool handlers (addTextToCanvas, clearCanvas, deleteSelectedObject, etc.)
                ...
                <VStack spacing={6} align="stretch">
                    ...
                    {!isCustomizeEnabled ? (
                        ...
                        <VStack spacing={4} align="stretch">
                            {/* Mockup Toggle (reintroduced with only Blank Tee active) */}
                            <RadioGroup onChange={setCurrentMockupType} value={currentMockupType} isDisabled={!isCustomizeEnabled}>
                                <Stack direction="row" spacing={4} justifyContent="center" mb={4}>
                                    ...
                                    {/* 'On Model' button, disabled for now */}
                                    <Button size="sm" colorScheme={currentMockupType === 'man' ? 'brandAccentYellow' : 'gray'} onClick={() => setCurrentMockupType('man')} isDisabled={true}>On Model</Button>
                                </Stack>
                            </RadioGroup>
