...
import {
    ...
    Collapse, useDisclosure, Flex, Card, CardBody, CardHeader
...
export default function Profile() {
  ...
  const handleBillingSameAsShippingChange = (e) => {
    ...
    if (isChecked) { setForm(prev => ({ ...prev, billingAddress: { ...prev.shippingAddress } })); }
  };
  const handleSave = async () => {
    setIsSaving(true);
    const getFinalAddress = (address) => isAddressEmpty(address) ? null : address;
    const payload = {
        username: form.username, firstName: form.firstName, lastName: form.lastName,
        shippingAddress: getFinalAddress(form.shippingAddress),
        billingAddress: billingSameAsShipping ? getFinalAddress(form.shippingAddress) : getFinalAddress(form.billingAddress),
    };
    try {
      ...
      toast({ title: "Error Saving Profile", description: error.response?.data?.message, status: "error" });
    } finally { setIsSaving(false); }
  };
  const handleCancel = () => { /* Logic to revert form state */ setEditing(false); };
  const handlePasswordFormChange = (e) => { const { name, value } = e.target; setPasswordForm(prev => ({ ...prev, [name]: value })); };
  const handleChangePassword = async (e) => { /* ... unchanged ... */ };

  if (loadingAuth || isLoading) {
    return <VStack justifyContent="center" minH="60vh"><Spinner size="xl" /></VStack>;
  }
  if (!user) {
  ...
  const renderAddressFields = (addressType) => (
    <VStack spacing={4} align="stretch" w="100%">
      ...
      <FormControl><FormLabel>Street Address 1:</FormLabel><Input name="street1" value={form[addressType]?.street1 || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} /></FormControl>
      <FormControl><FormLabel>Street Address 2:</FormLabel><Input name="street2" value={form[addressType]?.street2 || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} /></FormControl>
      <SimpleGrid columns={3} spacing={4}>
        <FormControl><FormLabel>City:</FormLabel><Input name="city" value={form[addressType]?.city || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} /></FormControl>
        <FormControl><FormLabel>State:</FormLabel><Input name="state" value={form[addressType]?.state || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} /></FormControl>
        <FormControl><FormLabel>Zip Code:</FormLabel><Input name="zipCode" value={form[addressType]?.zipCode || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} /></FormControl>
      </SimpleGrid>
      ...
      <FormControl><FormLabel>Phone:</FormLabel><Input name="phone" type="tel" value={form[addressType]?.phone || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} /></FormControl>
    </VStack>
  );
  ...
  return (
    <Box>
      ...
      <VStack spacing={6} align="stretch">
        ...
        <Card><CardBody>
            <Heading as="h2" size="lg">Shipping Address</Heading>
        ...
        <Stack direction="row" spacing={4} mt={6}>
            {!editing ? (<Button onClick={() => setEditing(true)} colorScheme="brandAccentYellow" leftIcon={<Icon as={FaEdit}/>}>Edit Profile</Button>) : (
              <>
                <Button onClick={handleSave} colorScheme="brandAccentOrange" isLoading={isSaving} leftIcon={<Icon as={FaSave}/>}>Save Profile</Button>
                <Button onClick={handleCancel} variant="ghost" leftIcon={<Icon as={FaTimes}/>}>Cancel</Button>
              </>
            )}
        </Stack>
        ...
        <Card><CardBody>
            <Flex as="button" onClick={onPasswordSectionToggle} align="center" justify="space-between" w="100%">
                <Heading as="h2" size="lg"><Icon as={FaKey} mr={3} />Change Password</Heading>
                <Icon as={isPasswordSectionOpen ? FaChevronUp : FaChevronDown} />
            </Flex>
            <Collapse in={isPasswordSectionOpen} animateOpacity>
                <Box as="form" onSubmit={handleChangePassword} pt={4}>
                    <VStack spacing={4} align="stretch">
