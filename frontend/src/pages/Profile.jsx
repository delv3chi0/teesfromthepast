...
// Helper to check if an address object is effectively empty
...
export default function Profile() {
      ...
      if (name === "firstName" || name === "lastName") {
        ...
        if (billingSameAsShipping) {
            if (!prevForm.shippingAddress.recipientName || prevForm.shippingAddress.recipientName === oldRecipientName) {
                ...
                newForm.billingAddress.recipientName = newRecipientName;
            }
        } else {
  ...
  if (authStillLoading || isLoading) {
    return <Box textAlign="center" mt={20} py={10}><Spinner size="xl" color="brand.primary" /><Text mt={4} color="brand.textDark">Loading Profile…</Text></Box>;
  }
  if (!user) {
    return <Box textAlign="center" mt={20} px={4}><Text color="brand.textDark" fontSize="lg">Could not load profile. You may need to log in again.</Text><Button mt={4} bg="brand.accentYellow" color="brand.textDark" _hover={{ bg: "brand.accentYellowHover"}} borderRadius="full" size="lg" onClick={() => navigate('/login')}>Go to Login</Button></Box>;
  }
  ...
  const renderAddressFields = (addressType, legend, includeNameFields = false) => (
    <VStack spacing={4} align="stretch" w="100%">
      ...
      <FormControl id={`${addressType}RecipientName`}>
        <FormLabel fontWeight="medium" color="brand.textDark" fontSize={{ base: "sm", md: "md" }}>Recipient Name:</FormLabel>
        <Input name="recipientName" placeholder="Full Name for Delivery/Billing" value={form[addressType]?.recipientName || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} bg={(!editing || (addressType === 'billingAddress' && billingSameAsShipping)) ? "gray.100" : "white"} borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md" size="lg" autoComplete={`${addressType === 'shippingAddress' ? 'shipping name' : 'billing name'}`}/>
      </FormControl>
      <FormControl id={`${addressType}Street1`}>
        <FormLabel fontWeight="medium" color="brand.textDark" fontSize={{ base: "sm", md: "md" }}>Street Address 1:</FormLabel>
        <Input name="street1" placeholder="123 Main St" value={form[addressType]?.street1 || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} bg={(!editing || (addressType === 'billingAddress' && billingSameAsShipping)) ? "gray.100" : "white"} borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md" size="lg" autoComplete={`${addressType === 'shippingAddress' ? 'shipping address-line1' : 'billing address-line1'}`}/>
      </FormControl>
      <FormControl id={`${addressType}Street2`}>
        <FormLabel fontWeight="medium" color="brand.textDark" fontSize={{ base: "sm", md: "md" }}>Street Address 2 (Apt, Suite):</FormLabel>
      ...
      <SimpleGrid columns={{ base: 1, sm: 3 }} spacing={4}>
        <FormControl id={`${addressType}City`}>
          <FormLabel fontWeight="medium" color="brand.textDark" fontSize={{ base: "sm", md: "md" }}>City:</FormLabel>
          <Input name="city" placeholder="City" value={form[addressType]?.city || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} bg={(!editing || (addressType === 'billingAddress' && billingSameAsShipping)) ? "gray.100" : "white"} borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md" size="lg" autoComplete={`${addressType === 'shippingAddress' ? 'shipping address-level2' : 'billing address-level2'}`}/>
        </FormControl>
        <FormControl id={`${addressType}State`}>
          <FormLabel fontWeight="medium" color="brand.textDark" fontSize={{ base: "sm", md: "md" }}>State/Province:</FormLabel>
          <Input name="state" placeholder="State/Province" value={form[addressType]?.state || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} bg={(!editing || (addressType === 'billingAddress' && billingSameAsShipping)) ? "gray.100" : "white"} borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md" size="lg" autoComplete={`${addressType === 'shippingAddress' ? 'shipping address-level1' : 'billing address-level1'}`}/>
        </FormControl>
        <FormControl id={`${addressType}ZipCode`}>
          <FormLabel fontWeight="medium" color="brand.textDark" fontSize={{ base: "sm", md: "md" }}>Zip/Postal Code:</FormLabel>
          <Input name="zipCode" placeholder="Zip/Postal Code" value={form[addressType]?.zipCode || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} bg={(!editing || (addressType === 'billingAddress' && billingSameAsShipping)) ? "gray.100" : "white"} borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md" size="lg" autoComplete={`${addressType === 'shippingAddress' ? 'shipping postal-code' : 'billing postal-code'}`}/>
        </FormControl>
      </SimpleGrid>
      <FormControl id={`${addressType}Country`}>
        <FormLabel fontWeight="medium" color="brand.textDark" fontSize={{ base: "sm", md: "md" }}>Country:</FormLabel>
        <Input name="country" placeholder="Country" value={form[addressType]?.country || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} bg={(!editing || (addressType === 'billingAddress' && billingSameAsShipping)) ? "gray.100" : "white"} borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md" size="lg" autoComplete={`${addressType === 'shippingAddress' ? 'shipping country-name' : 'billing country-name'}`}/>
      </FormControl>
      <FormControl id={`${addressType}Phone`}>
        <FormLabel fontWeight="medium" color="brand.textDark" fontSize={{ base: "sm", md: "md" }}>Phone (Optional):</FormLabel>
        <Input name="phone" type="tel" placeholder="Phone Number" value={form[addressType]?.phone || ''} onChange={(e) => handleAddressChange(e, addressType)} isReadOnly={!editing || (addressType === 'billingAddress' && billingSameAsShipping)} bg={(!editing || (addressType === 'billingAddress' && billingSameAsShipping)) ? "gray.100" : "white"} borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md" size="lg" autoComplete={`${addressType === 'shippingAddress' ? 'shipping tel' : 'billing tel'}`}/>
      </FormControl>
    </VStack>
  );
  ...
  return (
    <Box maxW="xl" p={{base: 4, md: 8}} borderWidth="1px" borderRadius="xl" shadow="xl" bg="brand.paper" mx="auto">
      ...
      <Box w="100%">
        <Flex
            as="button"
            onClick={onPasswordSectionToggle}
            align="center"
              ...
              <FormControl id="currentPassword-profile">
                ...
                <InputGroup size="md">
                  <Input
                    ...
                    value={passwordForm.currentPassword} onChange={handlePasswordFormChange}
                    placeholder="Enter your current password"
                    bg="white" borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md"
                  />
                  <InputRightElement>
                    <ChakraIconButton variant="ghost" icon={showCurrentPassword ? <FaEyeSlash /> : <FaEye />} onClick={() => setShowCurrentPassword(!showCurrentPassword)} aria-label="Toggle current password visibility"/>
                  </InputRightElement>
                </InputGroup>
              </FormControl>
              <FormControl id="newPassword-profile">
                <FormLabel fontWeight="semibold" color="brand.textDark">New Password:</FormLabel>
                <InputGroup size="md">
                  <Input
                    name="newPassword" type={showNewPassword ? "text" : "password"}
                    value={passwordForm.newPassword} onChange={handlePasswordFormChange}
                    placeholder="Min. 6 characters"
                    bg="white" borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md"
                  />
                  <InputRightElement>
                    <ChakraIconButton variant="ghost" icon={showNewPassword ? <FaEyeSlash /> : <FaEye />} onClick={() => setShowNewPassword(!showNewPassword)} aria-label="Toggle new password visibility"/>
                  </InputRightElement>
                </InputGroup>
              </FormControl>
              <FormControl id="confirmNewPassword-profile">
                <FormLabel fontWeight="semibold" color="brand.textDark">Confirm New Password:</FormLabel>
                <InputGroup size="md">
                  <Input
                    name="confirmNewPassword" type={showConfirmNewPassword ? "text" : "password"}
                    value={passwordForm.confirmNewPassword} onChange={handlePasswordFormChange}
                    placeholder="Confirm new password"
                    bg="white" borderColor="brand.secondary" focusBorderColor="brand.primaryDark" borderRadius="md"
                  />
                  <InputRightElement>
                    <ChakraIconButton variant="ghost" icon={showConfirmNewPassword ? <FaEyeSlash /> : <FaEye />} onClick={() => setShowConfirmNewPassword(!showConfirmNewPassword)} aria-label="Toggle confirm new password visibility"/>
                  </InputRightElement>
                </InputGroup>
              </FormControl>
              <Button
                type="submit" {...primaryButtonStyle}
                w={{ base: "full", md: "auto" }} alignSelf={{base: "stretch", md: "flex-start"}}
                isLoading={isPasswordSaving} loadingText="Updating..." leftIcon={<Icon as={FaSave} />} mt={2}
              >
                Update Password
              </Button>
            </VStack>
