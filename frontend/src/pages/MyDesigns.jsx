...
export default function MyDesigns() {
  ...
  const fetchDesigns = () => {
    if (user) {
      ...
      client.get('/mydesigns')
        ...
        setLoading(false);
        // setError("You need to be logged in to see your designs."); // Handled by PrivateRoute or loading state
    }
  };
  ...
  const handleConfirmSubmitToContest = async () => {
    ...
    try {
      ...
      onImageModalClose(); 
      fetchDesigns(); // Re-fetch designs to reflect any changes (e.g., submission status)
    } catch (err) {
          ...
          <ModalContent bg="brand.paper" borderRadius="lg">
            ...
            <ModalFooter borderTopWidth="1px" borderColor="gray.200">
              <Button 
                ...
                isDisabled={isSubmitting /* || selectedDesign.isSubmittedForContest - add this back if you have that field and logic */} 
                leftIcon={<Icon as={FaTrophy} />} 
                borderRadius="full" px={6}
              >
                {/* selectedDesign.isSubmittedForContest ? "Already Submitted" : "Submit to Contest" - add this back with field */}
                Submit to Contest
              </Button>
